    ...
    const errorMap: z.ZodErrorMap = (error, ctx) => {
      ...
      switch (error.code) {
        ...
        case ZodIssueCode.too_small:
          message = t(`zodI18n.errors.too_small.${error.type}.${error.exact ? 'exact' : error.inclusive ? 'inclusive' : 'not_inclusive'}`, {
            minimum: error.type === 'date' ? d(new Date(error.minimum as number), dateFormat) : error.minimum,
          })
          break
        case ZodIssueCode.too_big:
          message = t(`zodI18n.errors.too_big.${error.type}.${error.exact ? 'exact' : error.inclusive ? 'inclusive' : 'not_inclusive'}`, {
